...
fun pef(x: DoubleArray, fs: Int): FloatArray {
    ...
    val numHopsFinal=Math.ceil(((r - windowLength) / hopLength).toDouble())+oneCast+1

    val N:Int=windowLength.toInt()
    val hopSize:Int= hopLength.toInt()
    val numHops:Int= (Math.ceil(((r - N) / hopSize).toDouble())+oneCast).toInt()+1
...
//до этого момента правильно
    ...
    val f0=FloatArray(locs.size)
    for ( i in 0 until f0.size){
        f0[i]=logSpacedFrequency[locs[i]]
    }
